...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}•{x}] ├──ENTER YOUR COOKIE :{asu} ')
		...
		print(f'├── {x}[{h}•{x}]{h} LOGIN SUCCESSFULL !{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'├── %s[%sx%s]%s LOGIN FAILD !!!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tID YOU : [green] '+str(my_id)))
	print(f'├──[IP YOUR] : {ip}')
	print(f'├──[GITHUB]  : {gh}')
	print('│')
	print('├──[1] CRAKING START ')
	print('├──[2] CRACK SUCCESSFUL ')
	print('├──[3] EXIT TOOL       ')
	...
	if ilmanramdhanirahman in ['1']:
		...
		print('├──[•] Cookie out succeeded')
		exit()
	else:
		print('├──[•] Select Your Option ')
		back()
def error():
	print(f'{k}├──[•] Sorry this feature is still under fixing {x}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'├──[•] 1. RESULT {h}OK{x} YOU ')
	print(f'├──[•] 2. RESULT {k}CP{x} YOU ')
	print(f'├──[•] 3. RETURN	')
	kz = input(f'\n[•] SELECT : ')
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('├─__YOU HAVE NO RESULTS CP] ')
			time.sleep(2)
			back()
		else:
			cih = 0
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('├──[YOU DONT HAVE THE FILES OK]')
			time.sleep(2)
			back()
		else:
			cih = 0
